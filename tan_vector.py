...
# pcd.plot()
points_to_remove = []
if(tooh_number>=6and tooh_number<=10):
    max_deri_value = np.amax(deri_arr) * 0.8
    threshold = 2.95
else:
...
# arrow_tan
plot.add_mesh(arrow_tan,color="navy")
plot.add_mesh(pcd_group_a,color='red')
plot.add_mesh(pcd_group_b,color='blue')
plot.add_mesh(tooth)
plot.add_mesh(line,color='yellow',line_width=5)
plot.show()
print(tan_vector)
print(tan_normal)
